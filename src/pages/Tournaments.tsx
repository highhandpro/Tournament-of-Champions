...
export const Tournaments: React.FC<TournamentsProps> = ({
  ...
  useEffect(() => {
    if (activeTournament) {
      ...
      if (activeTournament.seating) {
        ...
        const savedSeating = localStorage.getItem(`tournament-of-champions_seating_${activeTournament.id}`);
        if (savedSeating) {
          try {
      ...
      if (activeTournament.dealers) {
        ...
        const savedDealers = localStorage.getItem(`tournament-of-champions_dealers_${activeTournament.id}`);
        if (savedDealers) {
          try {
      ...
      if (activeTournament.preassignedDealers) {
        ...
        const savedPreassigned = localStorage.getItem(`tournament-of-champions_preassigned_dealers_${activeTournament.id}`);
        if (savedPreassigned) {
          try {
  ...
  const toggleDealerStatus = (playerId: string, tableName: string) => {
    ...
    localStorage.setItem(`tournament-of-champions_dealers_${activeTournament.id}`, JSON.stringify(newDealers));
    setSeating(newSeating);
    localStorage.setItem(`tournament-of-champions_seating_${activeTournament.id}`, JSON.stringify(newSeating));
    setPreassignedDealers(updatedPreassigned);
    localStorage.setItem(`tournament-of-champions_preassigned_dealers_${activeTournament.id}`, JSON.stringify(updatedPreassigned));
  ...
  const toggleCheckedInDealer = (playerId: string) => {
    ...
    if (hasSeating) {
      ...
      localStorage.setItem(`tournament-of-champions_preassigned_dealers_${activeTournament.id}`, JSON.stringify(updated));
  ...
  const generateSeating = () => {
    ...
    localStorage.setItem(`tournament-of-champions_seating_${activeTournament.id}`, JSON.stringify(bestSeating));
    
    setDealers(bestDealers);
    localStorage.setItem(`tournament-of-champions_dealers_${activeTournament.id}`, JSON.stringify(bestDealers));
  ...
  const movePlayerTable = (playerId: string, sourceTable: string, targetTable: string) => {
    ...
    setSeating(updated);
    localStorage.setItem(`tournament-of-champions_seating_${activeTournament!.id}`, JSON.stringify(updated));
    setDealers(updatedDealers);
    localStorage.setItem(`tournament-of-champions_dealers_${activeTournament!.id}`, JSON.stringify(updatedDealers));
  ...
  const submitLateEntry = () => {
    ...
    triggerCheckInFlow(selectedLateMemberId, async (phone) => {
      ...
      localStorage.setItem(`tournament-of-champions_seating_${activeTournament.id}`, JSON.stringify(updatedSeating));
      ...
      <div className="animate-slide-up" style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
        ...
        <div style={{ display: 'flex', justifySelf: 'stretch', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-secondary)', marginTop: '4px' }}>
              Create, search, or administer Tournament of Champions club tournaments.
            </p>
          </div>
          {!isSubAdmin && (
                  ...
                  <div key={pageIdx} className="print-page-layout preview-page-sheet">
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #000', paddingBottom: '8px', marginBottom: '16px' }}>
                      <h3 style={{ fontSize: '14pt', fontWeight: 800, margin: 0, color: '#000' }}>
                        {printType === 'signin' ? 'Tournament of Champions Club - Player Sign-In Sheet' : `${activeTournament.name} - TD Score Sheet`}
                      </h3>
