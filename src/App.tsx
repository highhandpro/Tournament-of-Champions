...
function App() {
  ...
  const [portalMode, setPortalMode] = useState<'player' | 'admin'>(() => {
    const saved = localStorage.getItem('tournament-of-champions_portal_mode');
    const isAdminAuth = sessionStorage.getItem('tournament-of-champions_admin_auth') === 'true';
  ...
  const [loggedInMemberId, setLoggedInMemberId] = useState<string | null>(() => {
    return localStorage.getItem('tournament-of-champions_logged_in_member_id') || null;
  });
  ...
  const [isAdminAuthenticated, setIsAdminAuthenticated] = useState(() => {
    return sessionStorage.getItem('tournament-of-champions_admin_auth') === 'true';
  });
  const [adminEmail, setAdminEmail] = useState<string | null>(() => {
    return sessionStorage.getItem('tournament-of-champions_admin_email') || null;
  });
  const [isSubAdmin, setIsSubAdmin] = useState(() => {
    const role = sessionStorage.getItem('tournament-of-champions_admin_role');
    return role ? role === 'admin' : sessionStorage.getItem('tournament-of-champions_admin_sub') === 'true';
  });
  const [isTournamentDirector, setIsTournamentDirector] = useState(() => {
    return sessionStorage.getItem('tournament-of-champions_admin_role') === 'tournament-director';
  });
  const [isChiefAdmin, setIsChiefAdmin] = useState(() => {
    return sessionStorage.getItem('tournament-of-champions_admin_role') === 'chief-admin';
  });
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((user) => {
      if (user) {
        sessionStorage.setItem('tournament-of-champions_admin_auth', 'true');
        sessionStorage.setItem('tournament-of-champions_admin_email', user.email || '');
        setIsAdminAuthenticated(true);
        setAdminEmail(user.email || '');
      } else {
        sessionStorage.removeItem('tournament-of-champions_admin_auth');
        sessionStorage.removeItem('tournament-of-champions_admin_email');
  ...
  useEffect(() => {
    if (adminEmail) {
      ...
      sessionStorage.setItem('tournament-of-champions_admin_role', role);
      sessionStorage.setItem('tournament-of-champions_admin_sub', role === 'admin' ? 'true' : 'false');
    } else {
      setIsSubAdmin(false);
      setIsTournamentDirector(false);
      setIsChiefAdmin(false);
      sessionStorage.removeItem('tournament-of-champions_admin_role');
      sessionStorage.setItem('tournament-of-champions_admin_sub', 'false');
    }
  ...
  const handleAdminPasswordSubmit = async (e: React.FormEvent) => {
    ...
    if (isPrimaryChief && pass === '482-917') {
      ...
      sessionStorage.setItem('tournament-of-champions_admin_auth', 'true');
      sessionStorage.setItem('tournament-of-champions_admin_email', cleanEmail);
      sessionStorage.setItem('tournament-of-champions_admin_role', role);
      sessionStorage.setItem('tournament-of-champions_admin_sub', 'false');
      setIsAdminAuthenticated(true);
  ...
  const handleAdminLogout = async () => {
    try {
      ...
      sessionStorage.removeItem('tournament-of-champions_admin_auth');
      sessionStorage.removeItem('tournament-of-champions_admin_email');
      sessionStorage.removeItem('tournament-of-champions_admin_sub');
      sessionStorage.removeItem('tournament-of-champions_admin_role');
      setIsAdminAuthenticated(false);
      setAdminEmail(null);
      setIsSubAdmin(false);
  ...
  useEffect(() => {
    localStorage.setItem('tournament-of-champions_portal_mode', portalMode);
  }, [portalMode]);

  useEffect(() => {
    if (loggedInMemberId) {
      localStorage.setItem('tournament-of-champions_logged_in_member_id', loggedInMemberId);
    } else {
      localStorage.removeItem('tournament-of-champions_logged_in_member_id');
    }
  }, [loggedInMemberId]);
